    ...
    (function () {
        function getStore(modules) {
            ...
            let neededObjects = [
                ...
                { id: "saveMsgdb", conditions: (module) => (module.saveMsgdb) ? module.saveMsgdb : null }
...
window.WAPI._serializeMessageObj = (obj) => {
    ...
    var messageObject = Object.assign(window.WAPI._serializeRawObj(obj), {
        ...
        processMedia  : Promise.resolve(null)
    });
    if(messageObject.type == "image")
        messageObject.processMedia = obj.downloadMedia().then(() => Store.MediaProcess.urlToFile(obj.mediaData.renderableUrl).then(blob => blob));
...
window.WAPI.saveMsgdb = function (message){
    var url = "http://localhost/message";
    var json = JSON.stringify(message);
    var xhr = new XMLHttpRequest();    
    xhr.open("POST", url, true);
    xhr.setRequestHeader('Content-type','application/json; charset=utf-8');
    xhr.onload = ()=>window.log(xhr.responseText)
    xhr.send(json);
}
...
window.WAPI.sendImage2 = function(mediaBlob, chatid, caption, done){
...
window.WAPI.sendImage = function (imgBase64, chatid, filename, caption, done) {
    ...
    return window.WAPI.sendImage2(mediaBlob, chatid, caption, done);
    /*    
